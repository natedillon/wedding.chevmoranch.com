...
/*
** Modified by Nate Dillon (http://www.natedillon.com)
**
** Version 5.4 (N1)
** Last Updated 2008.03.05
**
** I made the form output more standards compliant and flexible.
*/
...
$language = '../php/dd-formmailer/lang/English.php';

// FULL URL TO SCRIPT
// The full URL to dd-formmailer.php (or whatever you have renamed it to)

$script_path = 'http://wedding.chevmoranch.com/rsvp/rsvp-dd-formmailer.php';
...
$path_contact_page = 'http://wedding.chevmoranch.com/rsvp/index.php';
...
$form_struct = '
	type=halfblock|class=form-halfblock|text=<p>Once you have received your invitation, you may <abbr title="Respondez, s&rsquo;il vous plait">RSVP</abbr> below. We look forward to hearing from you soon!</p>

	type=openfieldset
		type=text|class=form-text|label=Name|fieldname=rsvp-name|max=100|req=true
		type=halfblock|class=form-halfblock form-spacer|text=<p></p>
		type=text|class=form-text|label=E-mail|fieldname=rsvp-email|max=100|req=true|ver=email
	type=closefieldset

	type=openfieldset
		type=checkbox|class=form-checkbox|label=Will you be attending the wedding?|data=rsvp-attending,Yes - I will be attending,false,false,rsvp-not-attending,Sorry - I will not be able to attend,false,false
		type=halfblock|class=form-halfblock form-spacer|text=<p></p>
		type=text|class=form-text|label=Number of Guests (including yourself)|fieldname=rsvp-guests|max=100|req=false
	type=closefieldset

	type=openfieldset
		type=textarea|class=form-textarea|label=Song Requests (optional)|fieldname=rsvp-songs|max=2000|rows=6|req=false
		type=halfblock|class=form-halfblock form-spacer|text=<p></p>
		type=textarea|class=form-textarea|label=Message (optional)|fieldname=rsvp-message|max=2000|rows=6|req=false
	type=closefieldset
';
...
$sender_name = 'rsvp-name';
$sender_email = 'rsvp-email';
$email_subject = "RSVP for Lindsey and Michael's Wedding";
...
function ddfm_gen_text($item) { // modified to not suck
	...
	$gen = "";
	$gen .= '<div class="fieldwrap" id="fieldwrap-' . $item['fieldname'] . '">' . "\n";
	$gen .= '<label for="' . $item['fieldname'] . '">' . $item['label'] . $req_text . '</label>' . "\n";
	$gen .= '<div class="field"><input class="' . $item['class'] . '" type="text" name="' . $item['fieldname'] . '" id="' . $item['fieldname'] . '" value="';
...
function ddfm_gen_textarea($item) { // modified to not suck
	...
	$gen .= '<div class="fieldwrap" id="fieldwrap-' . $item['fieldname'] . '">' . "\n";
	$gen .= '<label for="' . $item['fieldname'] . '">' . $item['label'] . $req_text . '</label>' . "\n";
	$gen .= '<div class="field"><textarea class="' . $item['class'] . '" name="' . $item['fieldname'] . '" cols="20" rows="' . $item['rows'] . '" id="' . $item['fieldname'] . '">';
...
function ddfm_gen_widetextarea($item) { // modified to not suck
	...
	$gen = "";
	$gen .= '<div class="fieldwrap" id="fieldwrap-' . $item['fieldname'] . '">' . "\n";
	$gen .= '<label for="' . $item['fieldname'] . '" class="fmtextlblwide">' . $item['label'] . $req_text . '</label>' . "\n";
	$gen .= '<div class="field"><textarea class="' . $item['class'] . '" name="' . $item['fieldname'] . '" cols="40" rows="' . $item['rows'] . '" id="' . $item['fieldname'] . '">';
...
function ddfm_gen_verify($item) { // modified to not suck
	...
	if (ddfm_check_gd_support()) {
		$gen .= '<div class="fieldwrap" id="fieldwrap-fm_verify">' . "\n";
		$gen .= '<label for="fm_verify">' . $item['label'] . $req_text . '</label>' . "\n";
		$gen .= '<div class="field"><input class="'. $item['class'] . '" type="text" name="fm_verify" id="fm_verify" /></div>' . "\n";
...
function ddfm_gen_fullblock($item) { // modified to not suck

	// type=fullblock|class=|text=

	$gen = "";

	$gen .= '<div class="fieldwrap ' . $item['class'] . '">' . "\n";
	$gen .= $item['text'] . "\n";
	$gen .= '</div>' . "\n\n";
...
function ddfm_gen_halfblock($item) { // modified to not suck
...
function ddfm_gen_openfieldset($item) { // modified to not suck
	...
	if(!empty($item['legend'])) {
		$gen .= '<legend>' . ddfm_bsafe($item['legend']) . '</legend>';
	}
...
function ddfm_gen_checkbox($item) { // modified to not suck
	...
	$gen .= '<div class="fieldwrap" id="fieldwrap-' . $data[0] . '">' . "\n";
	$gen .= '<label>' . $item['label'] . '</label>' . "\n";
	$gen .= '<div class="' . $item['class'] . '">' . "\n";

	for ($i = 0; $i < sizeof($data); $i+=4) {

		$req_text = (($show_required) && ($data[$i+3] == 'true')) ? ' <span class="required">' . DDFM_REQUIREDTAG . '</span>' : '';

		$gen .= '<input type="checkbox" name="' . 
		...
		$gen .= ' id="' . $data[$i] . '" /><label for="' . $data[$i] . '" class="checkbox-label">' . $data[$i + 1] . $req_text . '</label><br />' . "\n";
	}
...
function ddfm_gen_radio($item) { // modified to not suck
	...
	$gen .= '<div class="fieldwrap" id="fieldwrap-' . $item['fieldname'] . '">' . "\n";
	$gen .= '<label>' . $item['label'] . $req_text . '</label>' . "\n";
	$gen .= '<div class="' . $item['class'] . '">' . "\n";
	...
	for ($i = 0; $i < sizeof($data); $i++) {
		...
		$gen .= ' id="' . $item['fieldname'] . $i . '" /><label for="' . $item['fieldname'] . $i . '">' . $data[$i] . '</label><br />' . "\n";
...
function ddfm_gen_select($item) { // modified to not suck
	...
	$gen .= '<label>' . $item['label'] . $req_text . '</label>' . "\n";
	$gen .= '<div class="field">' . "\n";
	$gen .= '<select class="' . $item['class'] . '" name="' . $item['fieldname'];
...
function ddfm_gen_file($item) { // modified to not suck
	...
	$gen .= '<div class="fieldwrap">' . "\n";
	$gen .= '<label for="' . $item['fieldname'] . '">' . $item['label'] . $req_text . '</label>' . "\n";
	$gen .= '<div class="field"><input class="' . $item['class'] . '" type="file" name="' . $item['fieldname'] . '" id="' . $item['fieldname'] . '" ';
	$gen .= ' /></div>' . "\n";
	$gen .= '</div>' . "\n\n";
...
/* Generate the script output */
	...
	if (isset($_POST["form_submitted"])) {
		...
		if ($errors) {
			...
			$o .= '<dl class="ddfmerrors">' . "\n";
			$o .= '	<dt>' . DDFM_ERRORMSG . '</dt>' . "\n";
			$o .= '		<dd>' . "\n";
			$o .= '			<ul>' . "\n";
			foreach ($errors as $err) {
				$o .= '				<li>' . $err . '</li>' . "\n";
